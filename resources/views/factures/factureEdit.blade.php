...
<div class="row justify-content-center mt-2">
    <div class="col-10">
        <div class="card shadow">
            ...
            <div class="card-body">
                <form action="{{route('facture.update',$facture->id)}}" method="post" class="row g-3 needs-validation ">
                    @csrf
                    @method('PATCH')
                    <div class="row">
                        ...
                        <div class="col-md-12 my-2">
                            ...
                            <select class="form-select shadow @error('mission_id')is-invalid
                                ...
                                <option value="{{$mission->id}}" {{($mission->id == $facture->mission_id)? "selected" :
                        ...
                        {{-- <div class="col-md-12 my-2">
                            <label for="type_facture_id" class="form-label">Type de Facture</label>
                            <select class="form-select shadow @error('type_facture_id')is-invalid
                                @enderror" id="type_facture_id" required name="type_facture_id">
                                <option selected disabled value="">...</option>
                                @foreach ($typefactures as $typefact)
                                <option value="{{$typefact->id}}">{{$typefact->designation}}</option>
                                @endforeach
                            </select>
                            @error('type_facture_id')
                            <div class="invalid-feedback">{{$errors->first('type_facture_id')}}</div>
                            @enderror
                        </div> --}}
                        <div class="col-md-6 my-2">
                            ...
                            <input type="date" class="form-control shadow @error('date_facturation')is-invalid
                                ...
                                value="{{$facture->date_facturation }}" required name="date_facturation">
                        ...
                        <div class="col-md-6  my-2">
                            ...
                            <select class="form-select shadow @error('exercice')is-invalid
                                ...
                                <option value="{{ $exercice->id}}" {{($exercice->id == $facture->exercice_id) ?
                            ...
                            {{-- <tbody> --}}
                                ...
                                <tr class="cloning_row" id="0">
                                    ...
                                    <td colspan="4">
                                        <input type="text" id="prestation" class="form-control shadow"
                                            readonly="readonly" value="{{$facture->mission->prestation->designation}}">
                                    </td>
                                    <td>
                                ...
                                <tr>
                                    ...
                                    <td><input type="text" id="total" class="sub_total form-control shadow"
                                            readonly="readonly" value="{{$facture->montant}}"></td>
                                </tr>
...
/*     console.log($('#entreprise_id').val()); */
        var missionOld= $("#mission_id").val();
        var montantOld = $("#montant").val()
        var prestationOld = $("#prestation").val()
        console.log(missionOld);
    $('#mission_id').click(function (e) {

        var mission_id = $("#mission_id").val();
            if(mission_id === missionOld){
                $('#montant').val(montantOld);
                $('#total').val(montantOld);
                $('#prestation').val(prestationOld);
            }else{
            ...
            method:"POST",
            data:{mission_id:mission_id, _token:_token ,edit:"edit"},
            success:function(result)
                {
                console.log(result);
                console.log(result.totalFacture);
